...
Triggers on push/PR to `master`/`develop`. Three parallel jobs (`api-tests`, `api-tests-python`, `ui-tests`), each: checkout → set up runtime → install deps → run tests (Python suites run smoke then regression as separate steps) → upload artifacts (`if: always()` so failures still leave artifacts behind). Two downstream jobs `needs: [all three]`: `test-summary` (writes a pass/fail table to the run summary, `exit 1` if anything failed) and `allure-report` (merges all three suites' Allure results into one HTML report).
