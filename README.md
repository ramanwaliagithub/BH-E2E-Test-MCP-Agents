...
Setup and exact commands to run all three agents in both implementations are in [RUN.md](RUN.md), which also covers practices worth following any time you're running LLM agents against a real codebase (secrets, cost control, tool scoping, version pinning, review gates). A condensed technical summary — architecture decisions and real problems found while building this — is in [AGENTIC_NOTES.md](AGENTIC_NOTES.md). How (and how not) this would plug into CI/CD — trigger models, secrets scoping, cost control, why it's structurally separate from `tests.yml` — is in [CICD.md](CICD.md) (design only; nothing is enabled yet). Full command-by-command history is in [AGENTIC_BUILD_LOG.md](AGENTIC_BUILD_LOG.md).
